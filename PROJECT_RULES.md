...
- Follow project-specific style guide in `GSD-STYLE.md`
...
### Server Management
- After making changes, ALWAYS ensure you start a new server for testing (when applicable)
- Always kill all related servers from previous tests before starting a new one
- Verification must include proof of server restart and functionality

### Code Reuse
- Always search for existing code to iterate on instead of creating new code from scratch
- Use `/map` command to understand codebase before implementing
- Check `ARCHITECTURE.md` for existing components and patterns

### DRY Principle (Don't Repeat Yourself)
- Avoid code duplication whenever possible
- Before writing new code, check if similar logic or functionality already exists
- Refactor existing code to accommodate new use cases when appropriate

### Environment Awareness
- Write code that accounts for different environments: development, testing, and production
- Use environment variables for environment-specific configuration
- Never hardcode environment-specific values (URLs, keys, paths)

### Change Scope Control
- Ensure you only make requested changes
- Only make additional changes if:
  - You are certain they are well understood
  - They are directly related to the request
  - You document them in `DECISIONS.md`

### Bug Fix Discipline
- When fixing a bug, do not introduce a new pattern or technology without exhausting all options with the current implementation
- If you must introduce a new pattern:
  - Document the decision in `DECISIONS.md`
  - Ensure you remove the previous implementation to avoid duplicate logic
...
### Script Discipline
- Avoid writing scripts directly in files if possible
- Especially avoid scripts that will only run once
- Use temporary commands or existing tooling instead
- If a script is necessary, place it in `scripts/` folder with documentation
